    ...
    with get_db() as db:
        ...
        db.executescript(
            ...
            CREATE TABLE IF NOT EXISTS tid_records (
                ...
                CHECK (tid GLOB '[0-9]*' AND length(tid) BETWEEN 1 AND 30),
...
def get_photos(tid: str) -> list[sqlite3.Row]:
    with get_db() as db:
        return db.execute(
            ...
            "FROM tid_photos WHERE tid = ? ORDER BY photo_order, id",
            (tid,),
        ).fetchall()
...
def create_tid(tid: str, description: str, file_ids: list[str], uid: int) -> None:
    ...
    with get_db() as db:
        db.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?)",
            (tid, description, uid, uid, now, now),
        )
        db.executemany(
            "INSERT INTO tid_photos "
            "(tid, file_id, photo_order, uploaded_by, created_at) VALUES (?, ?, ?, ?, ?)",
            [(tid, file_id, index, uid, now) for index, file_id in enumerate(file_ids, start=1)],
        )
...
def replace_photos(tid: str, file_ids: list[str], uid: int) -> None:
    now = utc_now()
    with get_db() as db:
        db.execute("DELETE FROM tid_photos WHERE tid = ?", (tid,))
        db.executemany(
            "INSERT INTO tid_photos "
            "(tid, file_id, photo_order, uploaded_by, created_at) VALUES (?, ?, ?, ?, ?)",
            [(tid, file_id, index, uid, now) for index, file_id in enumerate(file_ids, start=1)],
        ...
        db.execute(
            "UPDATE tid_records SET updated_by = ?, updated_at = ? WHERE tid = ?",
            (uid, now, tid),
        )
...
def append_photos(tid: str, file_ids: list[str], uid: int) -> None:
    ...
    with get_db() as db:
        db.executemany(
            ...
            "(tid, file_id, photo_order, uploaded_by, created_at) VALUES (?, ?, ?, ?, ?)",
            [
                (tid, file_id, existing_count + index, uid, now)
...
def update_description(tid: str, description: str, uid: int) -> None:
    with get_db() as db:
        db.execute(
            "UPDATE tid_records SET description = ?, updated_by = ?, updated_at = ? WHERE tid = ?",
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.effective_message.reply_text(
        ...
        "Ketik <b>Daftar</b> untuk menambah TID, <b>Edit</b> untuk mengubah data, "
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.effective_message.reply_text(
        "Proses dibatalkan. Kirim TID atau ketik Daftar/Edit kapan saja."
    )
...
async def search_tid(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not tid:
        await update.effective_message.reply_text(
            "Format belum dikenali. Kirim TID berupa angka, atau ketik Daftar/Edit/Batal."
        )
        return
    try:
...
async def edit_tid_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    context.user_data["tid"] = tid
    await update.effective_message.reply_text(
        f"Pilih perubahan untuk TID <code>{tid}</code>:",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup([
...
def build_application() -> Application:
    ...
    register_conversation = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(register_from_callback, pattern=r"^register:\d+$"),
        ],
        states={
